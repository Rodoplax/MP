...
int nearest(Location location) {
    int i_near = 0;
    Location nearest = _locations[0];
    double min_dist  = nearest.distance(location);
    for (int i=1; i<_size; i++) {
        double dist = _locations[0].distance(location);
        if (dist<min_dist) {
            i_near = i;
            nearest = _locations[i];
            min_dist = nearest.distance(location);
        }
    }
    return i;
}
...
void load(std::istream is) {
    ...
    if (n<0) 
        throw std:out_of_range("load(std::istream is): Out of Range. Input is negative")
    
    else if (n>DIM_VECTOR_LOCATIONS) 
        throw std:out_of_range("load(std::istream is): Out of Range. Input exceeds vector max size")
